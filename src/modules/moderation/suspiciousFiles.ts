...
export async function moderateSuspiciousFiles(msg: Message<AnyTextableGuildChannel>) {
    if (msg.member?.roles.includes(Config.roles.regular) || msg.member?.roles.includes(Config.roles.fileWhitelist) || msg.member?.roles.includes(Config.roles.donor)) return false;
